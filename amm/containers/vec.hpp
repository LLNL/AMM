...
//! ----------------------------------------------------------------------------
//! //! code borrowed from trimesh2 libray
//! ----------------------------------------------------------------------------
/*
Szymon Rusinkiewicz
...
#define VEC_UNINITIALIZED ((void *) 0)
    ...
    // changed by Harsh to support 2D and 3D Octree points
    Vec(const T &x, const T &y) {
        VEC_STATIC_CHECK(D == 2 || D == 3);
        v[0] = x; v[1] = y;
        if (D != 2) {
            printf(" Dimensionality error! vec(x,y)\n");
            exit(1);
            //v[2] = 0;
        }
    }
    Vec(const T &x, const T &y, const T &z) {
        ...
        v[0] = x; v[1] = y;
        if (D != 3) {
            printf(" Dimensionality error! vec(x,y,z)\n");
            exit(1);
        }
        else {
            v[2] = z;
    ...
    template <class S> explicit Vec(const S &x)
        { for (size_type i = 0; i < D; i++) v[i] = (T)x[i]; }
...
#pragma omp critical
    ...
    inline const std::string str(void) const
    {
        std::stringstream ss;
        ss << "{" << v[0];
        for (size_type i = 1; i < D; i++) {
    ...
    // C string representation of vector
...
// These are new functions declared in namespace utils.
namespace amm::utils {
